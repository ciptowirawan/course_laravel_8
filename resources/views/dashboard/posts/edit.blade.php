...
<script>
    ...
    title.addEventListener('change', function() {
        fetch('/dashboard/posts/checkSlug?title=' + title.value)
            .then(response => response.json()) // { slug : "my-story" }
            .then(data => slug.value = data.slug)
            // .then(function(aaa) {
            //     slug.value = aaa.slug;
            });

    // preventing the attach files option in trix-editor js side from working
    document.addEventListemer('trix-file-accept', function(e) {
        e.preventDefault();
    })
</script>
@endsection